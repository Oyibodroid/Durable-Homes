...
export function Header() {
  ...
  const handleSignOut = async () => {
    try {
      // 1. Close the dropdown immediately so it feels responsive
      setUserOpen(false);
      setMobileOpen(false);
      ...
      // 3. Clear server tokens (wrapped in try/catch so it doesn't block logout)
      await fetch("/api/auth/clear-order-token", {
        method: "POST",
        // Optional: add a timeout so it doesn't hang the UI
        signal: AbortSignal.timeout(3000),
      }).catch((err) => console.error("Token clear failed:", err));

      // 4. Trigger NextAuth SignOut
      await signOut({ callbackUrl: "/" });
    } catch (error) {
      console.error("Sign out error:", error);
      // Fallback: force a signout even if the fetch failed
      await signOut({ callbackUrl: "/" });
    }
  };
              ...
              {session ? (
                <div className="relative hidden sm:block">
                  ...
                  >
                    ...
                    <ChevronDown
                      className={`h-3.5 w-3.5 transition-transform ${userOpen ? "rotate-180" : ""}`}
                    />
                  </button>
                  {userOpen && (
                    <>
              ...
              <div className="pt-3 border-t border-white/5">
                {session ? (
                  ...
                  <Link
                    href="/auth/signin"
                    onClick={() => setMobileOpen(false)}
                    className="block px-4 py-3 text-sm text-[#C9A84C] font-semibold"
                  >
                    Sign In
                  </Link>
                )}
