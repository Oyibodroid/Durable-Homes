...
function SectionHeader({
  ...
  return (
    <div
      id={id}
      className="flex items-start gap-4 mb-6 pb-5 border-b border-gray-100"
    >
    ...
    <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
          <Field label="Store Name">
            <input
              className={inputCls}
              value={s.storeName}
              onChange={up("storeName")}
              placeholder="Durable Homes"
            />
          </Field>
          <Field label="Store Email">
            <input
              ...
              type="email"
              value={s.storeEmail}
              onChange={up("storeEmail")}
              placeholder="sales@durablehomes.com"
            />
          </Field>
          ...
          <Field label="Country">
            <select
              className={selectCls}
              value={s.storeCountry}
              onChange={up("storeCountry")}
            >
          ...
          <Field
            ...
            hint="Used in invoices and legal documents"
          >
            <input
              className={inputCls}
              value={s.storeAddress}
              onChange={up("storeAddress")}
              placeholder="123 Construction Avenue"
            />
          </Field>
          ...
          >
            <div className="relative">
              ...
              <input
                ...
                min="0"
                step="1000"
                value={s.shippingFreeThreshold}
                onChange={up("shippingFreeThreshold")}
              />
            </div>
          </Field>
          <Field label="Standard Rate (₦)" hint="Default delivery cost">
            <div className="relative">
              ...
              <input
                ...
                min="0"
                step="500"
                value={s.shippingBaseRate}
                onChange={up("shippingBaseRate")}
              />
            </div>
          </Field>
          <Field label="Express Rate (₦)" hint="1–2 day delivery cost">
            <div className="relative">
              ...
              <input
                ...
                min="0"
                step="500"
                value={s.shippingExpressRate}
                onChange={up("shippingExpressRate")}
              />
            </div>
          </Field>
        </div>
          ...
          <div className="grid grid-cols-3 gap-3 text-sm">
            <div className="bg-white border border-gray-100 p-3">
              ...
              <p className="font-bold text-gray-900">
                ₦{Number(s.shippingFreeThreshold || 0).toLocaleString()}
              </p>
            </div>
            <div className="bg-white border border-gray-100 p-3">
              <p className="text-xs text-gray-400 mb-1">Standard delivery</p>
              <p className="font-bold text-gray-900">
                ₦{Number(s.shippingBaseRate || 0).toLocaleString()}
              </p>
            </div>
            <div className="bg-white border border-gray-100 p-3">
              <p className="text-xs text-gray-400 mb-1">Express delivery</p>
              <p className="font-bold text-gray-900">
                ₦{Number(s.shippingExpressRate || 0).toLocaleString()}
              </p>
            </div>
          </div>
        </div>
      ...
      >
        ...
        <div className="space-y-5">
          ...
          <div
            className={`transition-opacity ${s.taxEnabled === "true" ? "opacity-100" : "opacity-40 pointer-events-none"}`}
          >
          ...
          {s.taxEnabled === "true" && (
            <div className="bg-amber-50 border border-amber-100 px-4 py-3 flex items-start gap-3">
              ...
              <p className="text-sm text-amber-800">
                VAT of <strong>{s.taxRate}%</strong> will be added to every
                order at checkout.
              </p>
            </div>
          )}
      ...
      >
        ...
        <div className="space-y-5">
          <Field
            ...
            hint="Receives new order alerts and low stock warnings"
          >
            <input
          ...
          <div
            className={`transition-opacity ${s.lowStockAlerts === "true" ? "opacity-100" : "opacity-40 pointer-events-none"}`}
          >
            <Field
              label="Low Stock Threshold"
              hint="Alert when product quantity falls below this number"
            >
              <input
      ...
      >
        ...
        <div className="space-y-5">
          ...
          <Field
            ...
            hint="Shown to visitors on the maintenance page"
          >
            <textarea
      ...
      <div id="payments" className="bg-white border border-gray-100 p-6">
        ...
        <div className="space-y-4">
          {[
            {
              ...
              desc: "Used to initialise and verify Paystack payments",
            },
            {
              label: "PAYSTACK_PUBLIC_KEY",
              desc: "Embedded in the client for Paystack pop-up",
            },
          ].map(({ label, desc }) => (
            <div
              key={label}
              className="flex items-start gap-4 p-4 bg-[#faf9f6] border border-gray-100"
            >
          ...
          <div className="flex items-start gap-3 bg-[#faf9f6] border border-gray-100 px-4 py-4 mt-2">
            ...
            <div>
              <p className="text-sm font-semibold text-gray-900">
                Why aren't these editable here?
              </p>
              <p className="text-sm text-gray-500 mt-1 leading-relaxed">
                Payment API keys are stored in your{" "}
                <code className="bg-gray-100 px-1 rounded text-xs">.env</code>{" "}
                file and deployed as environment variables. Storing them in the
                database would be a security risk. Update them in your hosting
