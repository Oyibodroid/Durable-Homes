...
export default function CheckoutPage() {
  ...
  const shipping =
    subtotal >= settings.shippingFreeThreshold ? 0 : settings.shippingBaseRate;
          ...
          <div className="container mx-auto px-6 lg:px-12">
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <h1 className="font-display text-4xl text-white font-medium">
                  Complete Your Order
                </h1>
              </div>
            ...
            <div className="flex items-center gap-3 mt-6">
              {STEPS.map((s, i) => (
                <div key={s.key} className="flex items-center gap-3">
                  <div
                    className={`flex items-center gap-2 text-xs font-semibold ${step === s.key ? "text-[#C9A84C]" : STEPS.indexOf({ key: step } as any) > i ? "text-green-400" : "text-gray-600"}`}
                  >
                    <div
                      className={`w-6 h-6 rounded-full flex items-center justify-center text-[10px] font-black ${step === s.key ? "bg-[#C9A84C] text-[#111008]" : "bg-white/10 text-gray-500"}`}
                    >
                      {i + 1}
                    </div>
                    {s.label}
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div className="ml-auto flex gap-2">
                          {["VISA", "MC", "GT"].map((b) => (
                            <span
                              key={b}
                              className="text-[9px] font-black border border-gray-200 px-1.5 py-0.5 text-gray-500"
                            >
                              {b}
                            </span>
                          ))}
              ...
              <div className="bg-white border border-gray-100 sticky top-24">
                ...
                <div className="p-5">
                  ...
                  <div className="space-y-3 mb-5 max-h-64 overflow-y-auto">
                    {items.map((item) => {
                      // 1. Get the raw image reference
                      const firstImage = item.product.images?.[0];

                      // 2. Safely extract the URL string
                      // We check if it's an object with a 'url' property, or if it's already a string
                      const img =
                  ...
                  {shipping > 0 && (
                    <p className="text-xs text-gray-400 mt-3">
                      Add ₦
                      {(
                        settings.shippingFreeThreshold - subtotal
                      ).toLocaleString()}{" "}
