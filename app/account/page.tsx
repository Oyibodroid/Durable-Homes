...
import { ShoppingBag, Heart, Star, MapPin, ArrowRight, Package, Clock } from 'lucide-react'
...
export default async function AccountDashboard() {
  ...
  const [orders, wishlistCount, reviewCount, addressCount] = await Promise.all([
    prisma.order.findMany({
      ...
      include: { items: { take: 1, include: { product: { include: { images: { where: { isMain: true }, take: 1 } } } } } },
    }),
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {stats.map(({ label, value, icon: Icon, href }) => (
          <Link key={label} href={href}
            className="bg-white border border-gray-100 p-5 hover:border-[#C9A84C]/40 transition-all group"
      ...
      <div className="bg-white border border-gray-100">
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-50">
          ...
          <Link href="/account/orders" className="text-xs text-[#C9A84C] font-semibold hover:underline flex items-center gap-1">
        ...
        {orders.length === 0 ? (
          <div className="py-16 text-center">
            ...
            <Link href="/shop" className="mt-3 inline-block text-xs text-[#C9A84C] font-semibold hover:underline">
              Start shopping →
            </Link>
          </div>
        ) : (
              ...
              return (
                <Link key={order.id} href={`/account/orders/${order.id}`}
                  ...
                  <div className="text-right flex-shrink-0">
                    <p className="font-bold text-sm text-gray-900">₦{Number(order.total).toLocaleString()}</p>
                    <span className={`inline-block text-xs px-2 py-0.5 mt-1 border font-medium ${STATUS_COLORS[order.status] ?? 'bg-gray-50 text-gray-600 border-gray-200'}`}>
                      {order.status}
                    </span>
                  </div>
                </Link>
